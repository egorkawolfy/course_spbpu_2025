...
def __integr_func(
    ...
    # Плотность воды
    ro_w = calc_rho_w(ws, t)
    # Гравитационный градиент давления + градиент давлеиня на трение
    dp = calc_dp_dl(ro_w, mu_w, angle, q_ms, d_tub, roughness)
    # Температурный градиетн
    dt = temp_grad * 0.01
...
Q = 200 # дебит флюида, м3/cут
q_m3_sec = Q / 86400 # дебит флюида, м3/c
gamma_water = 1.0179115494421604 # относительная плотность по пресной воде с плотностью 1000 кг/м3, безразм
H = 2560.7358194324115 # измеренная глубина забоя скважины
d_tub = 0.08581831634516876 # диаметр НКТ, м
angle = 56.22825885617751 # угол наклона скважины к горизонтали, градусы
roughness = 0.0003935839727709461 # шероховатость трубы, м
p_wh = 151.32654595389886 * 101325 # давление на устье, атм
t_wh = 28.75610701819444 # температура на устье скважины, С
temp_grad = 2.947060639486247 # геотермический градиент, К/м * (1e-2)
h0 = 0
...
ax.invert_yaxis()
plt.legend()
plt.title("Распределение давления и температуры")
plt.show()

# VLP

q_massive = np.linspace(1, 400, 100)
...
for q in q_massive:
    ...
    p_bh_massive.append(p_bh_i / 101325)



# Формирование и сохранение результатов VLP расчета в json

vlp_data = {
    "q_liq": [float(q) for q in q_massive],
    "p_wf": [float(p) for p in p_bh_massive]
}

result_json = json.dumps(vlp_data, indent=2, ensure_ascii=False)
print(result_json)

with open('output_Volkov.json', 'w') as output_file:
    output_file.write(result_json)

# Построение графика
plt.plot(q_massive, p_bh_massive)
plt.xlabel("Дебит, м3/сут")
